...
func (g *grpc) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
	...
	for _, method := range service.Method {
		...
		g.P(`req := &gpbrpc.RPCRequest{Req: gpbrpc.RPCRequestPacket{ServiceName: "`, servName, `", FuncName: "`, method.GetName(), `", ReqPayload: string(buf), Context: _c.GetContent()}, Callback: cb, Exception: exp, Handle: _c}`)
	...
	for _, method := range service.Method {
		...
		g.P(`req := &gpbrpc.RPCRequest{Req: gpbrpc.RPCRequestPacket{ServiceName: "`, servName, `", FuncName: "`, method.GetName(), `", ReqPayload: string(buf), Context: _c.GetContent()}, Signal: sg, Handle: _c}`)
		g.P("_c.rpcclient.PushRequest(req)")
		...
		g.P(`if s.RspPayload != "" {`)
		g.P(`if err := proto.Unmarshal([]byte(s.RspPayload), out); err != nil {`)
	...
	g.P("NewHandle() TestServer")
	g.P("//record current content for sending rpcresponse async")
	g.P("SetCurrent(gpbrpc.Current)")
	g.P("//rpc request is need response immediately")
	g.P("SetResponse(bool)")
	g.P("//default return should be true")
	g.P("IsResponse() bool")
	g.P("}")
	g.P()
	...
	g.P("return &", servName, "{s}")
	g.P("}")
	g.P()

	g.P("func (s *", servName, ") NewHandle() gpbrpc.ServerInterface {")
	g.P("return &", servName, "{s.", servName, "Server.NewHandle()}")
	g.P("}")
	g.P()
	...
	if len(service.Method) > 0 {
		for i, method := range service.Method {
			...
			g.P("in:=&", inType, "{}")
			g.P(`if err = proto.Unmarshal([]byte(req.ReqPayload), in); err != nil {`)
			g.P(`ret = gpbrpc.GPBUnmarshalFailed`)
